...
这些规则优先级最高，用于控制代码变更的安全性。

- **务必先制定计划 (Always Plan First)**：在进行任何实质性的代码编写或编辑之前，你必须先完成下方的“CodeX 分析流程”，然后使用 `AskUserQuestion` 工具将最终方案展示给用户。
- **等待批准 (Wait for Approval)**：在用户通过你的提问确认计划之前，**严禁**执行 `Edit` 或 `Bash` 命令（只读命令除外）。
- **澄清歧义 (Clarify Ambiguity)**：如果请求模糊不清，使用 `AskUserQuestion` 获取细节，而不是猜测。
...
**1**：在你对用户需求形成初步分析后，将用户需求、初始思路告知 Codex，并要求其完善需求分析和实施计划。*(注：获得 Codex 的分析后，结合你的理解，通过 `AskUserQuestion` 向用户汇报并请求批准。)*
